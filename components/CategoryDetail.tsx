...
import { getCategoryWithImages, updateCategory, deleteCategory, uploadImage } from '../services/api';
import type { CategoryReadWithImages, CategoryUpdate, ImageRead, BodyUploadImage, ApiError } from '../types';
...
import { EditIcon, TrashIcon, UploadIcon, ImagePlaceholderIcon } from './icons'; // Added ImagePlaceholderIcon
...
const CategoryDetail: React.FC = () => {
  ...
  const fetchCategoryDetails = useCallback(async (openImageId?: string | null) => {
  ...
  useEffect(() => {
    if (!isLoading && category && category.images && !isImageDetailModalOpen && !initialImageOpened) {
      const queryParams = new URLSearchParams(location.search);
      const imageIdToOpen = queryParams.get('imageId');
      if (imageIdToOpen) {
  ...
  const handleImageDelete = (deletedImageId: string) => {
    setCategory(prev => {
      if (!prev || !prev.images) return prev;
      const wasThumbnail = (prev.thumbnail_url && (prev.thumbnail_url.includes(deletedImageId) || prev.thumbnail_url.includes(encodeURIComponent(deletedImageId)))) || 
  ...
  const handleNextImage = () => {
    if (category?.images && currentImageIndex !== null && category.images.length > 0 && currentImageIndex < category.images.length - 1) {
      const newIndex = currentImageIndex + 1;
      setSelectedImage(category.images[newIndex]);
      setCurrentImageIndex(newIndex);
    }
  ...
  if (thumbnailImageDetails) {
    dynamicImageContainerClasses = 'transition-all duration-300 ease-out group-hover:scale-103 group-hover:-translate-y-1 group-hover:shadow-xl';
  } else {
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      {error && <div className="mb-4"><ErrorDisplay error={error} onRetry={() => fetchCategoryDetails()} /></div>}
      ...
      >
        {isLoading && !category ? ( 
          <div className="animate-pulse">
            <div className={`w-full md:w-1/3 h-48 md:h-64 ${theme.skeletonBase} ${theme.card.rounded} mb-4`}></div>
            <div className={`w-1/2 h-8 sm:h-10 ${theme.skeletonBase} ${theme.card.rounded} mb-4`}></div>
            <div className={`w-full h-5 sm:h-6 ${theme.skeletonBase} ${theme.card.rounded} mb-2`}></div>
          ...
          <div className="md:flex md:items-start md:space-x-6">
              <div 
                ...
                aria-label={thumbnailImageDetails ? `View thumbnail image: ${thumbnailImageDetails.image.title || category.name}` : category.name}
              >
                {categoryThumbnailToDisplay && !categoryThumbnailLoadError ? (
                  ...
                  <div className={`w-full h-full flex items-center justify-center ${theme.skeletonBase} ${ (categoryThumbnailToDisplay && categoryThumbnailLoadError) ? theme.skeletonHighlight : '' }`}>
                    <ImagePlaceholderIcon className={`w-16 h-16 sm:w-20 sm:h-20 opacity-50 ${theme.card.secondaryText}`} />
                  </div>
                )}
                {thumbnailImageDetails && categoryThumbnailToDisplay && !categoryThumbnailLoadError && (
                  <div className={`absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-20 transition-all duration-300 flex items-center justify-center ${theme.card.rounded}`}>
                  </div>
                )}
              </div>
              <div className="md:w-2/3">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3 sm:mb-4">
                      <div className="mb-2 sm:mb-0">
                          <h1 className={`text-2xl sm:text-3xl md:text-4xl font-bold ${theme.text}`}>{category.name}</h1>
                      ...
                      {isAuthenticated && (
                        ...
                        <button
                            onClick={() => { setIsEditCategoryModalOpen(true); setFormError(null); }}
                  ...
                  <p className={`${theme.card.text} whitespace-pre-wrap text-sm sm:text-base`}>{category.description || 'No description provided for this category.'}</p>
              </div>
          </div>
        )}
      </div>
      ...
      >
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mt-6 sm:mt-8 mb-4 sm:mb-6 gap-3">
          <h2 className={`text-xl sm:text-2xl font-semibold ${theme.text}`}>Images in this Category ({(isLoading && !category?.images) ? '...' : category?.images?.length || 0})</h2>
          {isAuthenticated && (
            <button
              onClick={() => { setIsUploadImageModalOpen(true); setFormError(null); }}
        ...
        {isLoading && (!category || !category.images || category.images.length === 0) ? (
          ...
          !isLoading && <p className={`text-center ${theme.card.secondaryText} py-10 animate-fadeIn text-sm sm:text-base`}>No images found in this category. {isAuthenticated ? "Upload some!" : "Login to upload images."}</p>
        )}
      </div>
      ...
      {selectedImage && (
        <ImageDetailModal
          ...
          hasNextImage={category?.images && currentImageIndex !== null && currentImageIndex < category.images.length - 1}
      ...
      {isAuthenticated && category && isUploadImageModalOpen && (
        <Modal isOpen={isUploadImageModalOpen} onClose={() => setIsUploadImageModalOpen(false)} title="Upload New Image">
          <ImageUploadForm
