...
const ImageUploadForm: React.FC<ImageUploadFormProps> = ({ categoryId, onSubmit, onCancel, isLoading, error }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
      ...
      <div>
        ...
        <div className={`mt-1 flex flex-col items-center justify-center px-3 sm:px-6 pt-4 pb-4 sm:pt-5 sm:pb-6 border-2 ${theme.input.border.replace('border-', 'border-dashed border-')} ${theme.card.rounded}`}>
          <div className="space-y-1 text-center">
            {previewUrl ? (
              <img src={previewUrl} alt="Preview" className="mx-auto h-24 sm:h-32 w-auto object-contain rounded-md mb-2" />
            ) : (
              <UploadIcon className={`mx-auto h-10 w-10 sm:h-12 sm:w-12 ${theme.card.secondaryText}`} />
            )}
            <div className={`flex flex-col sm:flex-row text-xs sm:text-sm ${theme.card.secondaryText}`}>
              <label
                htmlFor="imageFile"
              ...
              >
                ...
                <input id="imageFile" name="imageFile" type="file" className="sr-only" onChange={handleFileChange} accept="image/*" required />
              </label>
              <p className="pl-0 sm:pl-1 mt-1 sm:mt-0">or drag and drop</p>
            </div>
            <p className={`text-xs ${theme.card.secondaryText} opacity-75`}>PNG, JPG, GIF up to 10MB</p>
          </div>
        </div>
         {file && <p className={`text-xs sm:text-sm ${theme.card.secondaryText} mt-1`}>Selected: {file.name}</p>}
      </div>

      <div>
        <label htmlFor="imageTitle" className={`block text-sm font-medium ${theme.card.text}`}>Title (Optional)</label>
        <input
          type="text"
          id="imageTitle"
          value={title}
      ...
      <div>
        <label htmlFor="imageDescription" className={`block text-sm font-medium ${theme.card.text}`}>Description (Optional)</label>
        <textarea
          id="imageDescription"
          value={description}
      ...
      <div>
        <label htmlFor="imageTags" className={`block text-sm font-medium ${theme.card.text}`}>Tags (Optional, comma-separated)</label>
        <input
          type="text"
          id="imageTags"
          value={tags}
