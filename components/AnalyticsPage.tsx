...
import { TimeIcon, TwitterIcon, DocumentTextIcon, XiaohongshuIcon, HeartFilledIcon, ShareIcon } from './icons'; 
...
const SocialCard: React.FC<SocialCardProps> = ({ icon, cardTitle, text, avatarSrc, userName, likes, shares, bgColorClass, textColorClass, iconColorClass }) => {
    const { theme } = useTheme();
    const finalIconColor = iconColorClass || textColorClass;
    return (
        <div className={`${bgColorClass} ${textColorClass} ${theme.card.rounded} p-4 sm:p-5 shadow-lg flex flex-col justify-between min-h-[200px] sm:min-h-[220px] h-full ${theme.card.border || ''} group-hover:shadow-xl transition-shadow duration-200`}>
            <div>
            ...
            <div className="flex justify-between items-center mt-auto">
                <div className="flex items-center">
                    <img src={avatarSrc} alt={userName} className="w-8 h-8 sm:w-9 sm:h-9 rounded-full mr-2 border border-current object-cover" />
...
const AnalyticsPage: React.FC = () => {
  ...
  const fetchData = useCallback(async <TData,>(
    fetchUrl: string,
  ...
  let timeIconColor = theme.brandColor;
  let timeValueColor = theme.brandColor;
  let timeIconContainerBg = 
    theme.name === 'Modern Clean Pro' ? 'bg-blue-100 dark:bg-blue-800/30' :
    theme.name === 'Nature Inspired' ? 'bg-lime-200 dark:bg-green-800/30' :
    theme.name === 'Neon Galaxy' ? 'bg-cyan-500/10 dark:bg-cyan-400/10' :
    theme.name === 'Arcade Flash' ? 'bg-yellow-300 dark:bg-yellow-700/30' :
    theme.name === 'RetroTech Dark' ? 'bg-emerald-400/10 dark:bg-emerald-500/10' :
  ...
  if (theme.name === 'Modern Clean Pro') {
    twitterCardColors = { bg: 'bg-blue-100 dark:bg-blue-900/50', text: 'text-blue-700 dark:text-blue-300', icon: 'text-blue-500 dark:text-blue-400' };
    xhsCardColors = { bg: 'bg-rose-100 dark:bg-rose-900/50', text: 'text-rose-700 dark:text-rose-300', icon: 'text-rose-500 dark:text-rose-400' };
  } else if (theme.name === 'Nature Inspired') {
    twitterCardColors = { bg: 'bg-lime-100 dark:bg-green-800/70', text: 'text-green-700 dark:text-lime-200', icon: 'text-green-600 dark:text-lime-300' };
    xhsCardColors = { bg: 'bg-orange-100 dark:bg-amber-800/70', text: 'text-orange-700 dark:text-orange-200', icon: 'text-orange-500 dark:text-amber-400' };
  } else if (theme.name === 'Neon Galaxy') {
    twitterCardColors = { bg: 'bg-cyan-900/50 dark:bg-cyan-700/20', text: 'text-cyan-200 dark:text-cyan-100', icon: 'text-cyan-400 dark:text-cyan-300' };
    xhsCardColors = { bg: 'bg-pink-900/50 dark:bg-pink-700/20', text: 'text-pink-200 dark:text-pink-100', icon: 'text-pink-400 dark:text-pink-300' };
  } else if (theme.name === 'Arcade Flash') {
    twitterCardColors = { bg: 'bg-yellow-300 dark:bg-yellow-700/80', text: 'text-yellow-900 dark:text-yellow-200', icon: 'text-yellow-700 dark:text-yellow-400' };
    xhsCardColors = { bg: 'bg-red-300 dark:bg-red-700/80', text: 'text-red-900 dark:text-red-200', icon: 'text-red-600 dark:text-red-400' };
  } else if (theme.name === 'RetroTech Dark') {
  ...
  const pieChartColors = useMemo(() => {
    ...
    const arcadeFlashColors = [
        '#EF4444', '#FACC15', '#F97316', '#F87171', '#FDE047', 
        '#FB923C', '#FEF3C7', '#DC2626', '#EAB308', '#FFFBEB'
    ];

    const retroTechDarkColors = [ // Mint green shades and neutral grays
        '#2EE59D', // emerald-400
        '#10B981', // emerald-500
        '#6EE7B7', // emerald-300
        '#059669', // emerald-600
  ...
  const mapThemeConfig = useMemo(() => {
    ...
    switch (theme.name) {
      case 'Nature Inspired':
        ...
        geoEmphasisAreaColor = isDarkMode ? theme.brandColor.replace('text-','bg-').replace('-300', '-500') : theme.brandColor.replace('text-','bg-').replace('-700', '-200');
        geoEmphasisBorderColor = isDarkMode ? theme.brandColor.replace('text-','border-').replace('-300', '-400') : theme.brandColor.replace('text-','border-').replace('-700', '-500');
      ...
      default:
        ...
        tooltipValueColor = isDarkMode ? theme.brandColor.split(" ")[1] : theme.brandColor.split(" ")[0]; // Handles dark:text-blue-400
        break;
    }
    return { visualMapColors, geoItemAreaColor, geoItemBorderColor, geoEmphasisAreaColor, geoEmphasisBorderColor, visualMapHandleColor, tooltipTitleColor, tooltipValueColor };
  }, [theme.name, theme.brandColor, isDarkMode]);
  
  return (
    <div className={`py-4 sm:py-5 ${theme.text} space-y-6`}>
      <div className={`flex flex-col ${theme.card.bg} ${theme.card.rounded} ${theme.card.shadow} ${theme.card.border || ''} p-4 sm:p-6`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-5 sm:gap-6">
        <div className={`lg:col-span-1 ${theme.card.bg} ${theme.card.rounded} ${theme.card.shadow} ${theme.card.border || ''} p-4 sm:p-6`}>
          <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-1 sm:mb-2">
            <div>
              ...
              <p className={`${theme.card.secondaryText} text-sm`}>Species with the highest observation counts.</p>
            </div>
          </div>
          {isLoadingTopBirds ? (
             <div className="flex justify-center items-center h-[360px] sm:h-[400px] md:h-[420px]"><LoadingSpinner /></div>
          ) : topBirdsError ? (
        ...
        <div className={`lg:col-span-3 ${theme.card.bg} ${theme.card.rounded} ${theme.card.shadow} ${theme.card.border || ''} p-4 sm:p-6`}>
      ...
      <div className={`w-full ${theme.card.bg} ${theme.card.rounded} ${theme.card.shadow} ${theme.card.border || ''} p-4 sm:p-6`}>
      ...
      <div className="w-full">
        {isLoadingSocialStats ? (
           <div className="flex flex-col sm:flex-row gap-5 sm:gap-6 h-full items-center justify-center"><LoadingSpinner /></div>
        ) : socialStatsError ? (
          ...
          <div className="flex flex-col sm:flex-row gap-5 sm:gap-6">
            <a 
              ...
              className={`flex-1 block group cursor-pointer ${theme.card.rounded} focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 ${theme.input.focusRing.replace('focus:ring-2', '').trim().split(' ').filter(c => !c.startsWith('focus:')).join(' ')} focus-visible:ring-offset-current`}
